...
def macsec_loganalyzer_ignore(loganalyzer, macsec_duthost):
    """
    Suppress error messages that are expected during dirty-restart recovery
    and are not indicative of real faults:

    - DNX SAI 'Invalid object ID': syncd polls SA attributes after orchagent
...
def test_dirty_container_kill_preserves_sak_consistency(
        macsec_duthost, ctrl_links, policy, cipher_suite, send_sci,
        force_dut_key_server):
    """
    ...
    force_dut_key_server is required so the DUT's freshly-restarted
    wpa_supplicant acts as MKA key server and generates a new SAK after
    restart.  Without this, a peer KS may redistribute the same SAK (no
    change detected), making step 6b vacuous.
    """
    duthost = macsec_duthost
...
def test_dirty_container_kill_recovers_encoding_an(
        macsec_duthost, profile_name, ctrl_links, policy, cipher_suite,
